...
| `send`                | implemented | `--surface <id>`                                    | `--text <text>`, `--bytes <base64>`, `--shell <mode>`, `--confirm`/`--no-confirm` (confirmed is the default, issue #88), `--timeout-ms <ms>`, `--force` (bypass the blocked-send gate, issue #93), `--wait`/`--wait-timeout-ms <ms>` (require observed working/blocked activity, issue #93) | none (exit 3 on `agent_blocked`; exit 1 on `input_ack_timeout`/`oversized_input`/`legacy_host_receipt_rejected`)                                                                          |
